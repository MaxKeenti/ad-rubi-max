...
- **Canonical sources, English** — `docs/CONTEXT.md`, `docs/adr/*.md`, `docs/implementation_plan.md`, `docs/grilling-session-*.md`. This is where decisions actually live. Edited freely as the design evolves.
- **Deliverables, Spanish** — `docs/entrega/0X-*/content.md` (with Typst wrappers in `main.typ`). Submitted to the professor. Must reflect the current state of the canonical sources.
...
1. **Diagnose drift.** Don't open every file blindly. Use git to scope the work:
   ...
   git log --since="$(git log -1 --format=%cd docs/entrega/)" \
           --name-only --oneline -- docs/CONTEXT.md docs/adr/ \
                                    docs/implementation_plan.md \
                                    'docs/grilling-session-*.md'
   ```
...
- A new ADR appeared in `docs/adr/` and you can't tell from context how to integrate it elsewhere.
...
- If a deliverable fails because cmarker chokes on a complex table, the fix is usually to simplify the table in markdown or hand-write that one section in Typst (per the (B-hybrid) lazy escape hatch documented in `docs/entrega/README.md`).
