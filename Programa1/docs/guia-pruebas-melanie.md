...
> versión "ejecutable" de `docs/entrega/08-plan-de-pruebas/content.md`
...
- [ ] **El APK firmado**, archivo `app-debug.apk`. Lo puedes encontrar en `app/build/outputs/apk/debug/app-debug.apk`.
...
1. Descarga el APK en el teléfono. En desarrollo se genera como
   `app/build/outputs/apk/debug/app-debug.apk` y puedes
   instalarlo con `adb install app/build/outputs/apk/debug/app-debug.apk`.
