...
- `docs/implementation_plan.md` Component 1 (Project Setup) and Component 10 (Navigation & Main Activity)
- `docs/entrega/03-arquitectura/content.md` § 2 (Stack tecnológico) and § 3 (Package structure)
...
- `build.gradle.kts` (project-level) — add Google Services plugin, Hilt plugin
- `app/build.gradle.kts` — add Firebase BOM + Firestore + Auth, Hilt, Navigation Compose, kapt, apply plugins
- `settings.gradle.kts` — version catalog entries
- `app/src/main/AndroidManifest.xml` — add INTERNET permission, set application class to `MangosApp`

Files to create:

- `app/src/main/java/com/example/mangos/MangosApp.kt` — `@HiltAndroidApp class MangosApp : Application()`
- `app/src/main/java/com/example/mangos/MainActivity.kt` — replace default with `@AndroidEntryPoint` Compose activity hosting a placeholder
...
- **`google-services.json` path** must be `app/google-services.json` (next to `app/build.gradle.kts`).
