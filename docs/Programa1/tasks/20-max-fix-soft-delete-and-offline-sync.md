...
`docs/Programa1/guia-pruebas-melanie.md` (corrida 30-31/05/2026, Xiaomi
15T, Android 16):
...
- `docs/Programa1/guia-pruebas-melanie.md` — secciones CP-06 (líneas
  261-287) y CP-07 (líneas 289-323) con la observación exacta de
  Melanie y la **Hoja de resultados** (líneas 501-522).
- `docs/Programa1/tasks/14-max-purchase-repository-real.md` —
  decisiones de diseño del repo (denormalización, ventana 24h,
  `serverWrittenAt`, `dateKey`). El fix no debe romper CP-02, CP-03,
  CP-04, CP-05, CP-11.
- `docs/Programa1/entrega/04-modelo-de-datos/content.md` § 2.3 —
  semántica de `deletedAt`/`deletedBy` y los tres timestamps.
- `docs/Programa1/adr/0002-server-side-authz.md` — el reloj de
  auditoría vive en `serverWrittenAt`. **No** estamos cambiando ese
  reloj; el cambio aquí es sobre `deletedAt`.
- `Programa1/app/src/main/java/com/example/mangos/data/repository/firestore/PurchaseRepositoryFirestoreImpl.kt` —
  los queries reactivos y el `softDelete`/`add`.
- `Programa1/app/src/main/java/com/example/mangos/ui/purchases/AddEditPurchaseViewModel.kt` —
  flujo de guardado, `isSaving`/`saveCompleted`.
- `Programa1/app/src/main/java/com/example/mangos/ui/dashboard/DashboardViewModel.kt` —
...
`Programa1/app/src/main/java/com/example/mangos/data/repository/fake/FakePurchaseRepository.kt`
...
- [ ] **Actualizar la hoja de resultados** en
  `docs/Programa1/guia-pruebas-melanie.md` (CP-06 y CP-07 a Pasa con
  link al PR del fix) **solo después** de re-correr con Melanie en el
  mismo Xiaomi 15T.
- [ ] **Actualizar `docs/Programa1/entrega/08-plan-de-pruebas/content.md`**
