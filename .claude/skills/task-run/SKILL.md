...
/task-run docs/Programa1/tasks/03-melanie-login.md
```

Resolve the input to a concrete task file:

- A number → match against `NN-*.md` files in `docs/Programa1/tasks/`. If
...
2. **Read every doc named in the Inputs section.** Don't skim — the
   ...
   - The relevant section of `docs/Programa1/entrega/*/content.md`
     (Spanish formal docs)
   - The relevant section of `docs/Programa1/CONTEXT.md`
   - Any ADRs (`docs/Programa1/adr/*.md`)
   - The interface files (`Programa1/app/src/main/java/com/example/mangos/data/repository/*.kt`)
...
- **"Compiles" / `./gradlew assembleDebug`**: run it. If the user has Gradle wrapper at `Programa1/gradlew`, use that.
